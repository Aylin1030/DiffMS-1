...
class Spec2MolDataModule(MolecularDataModule):
    ...
    def valency_count(self, max_n_nodes):
        ...
        if loader is None:
            # 如果所有数据集都是空的，返回默认值
            valencies[0] = 1.0
            return valencies / valencies.sum()
    ...
    def node_counts(self, max_nodes_possible=150):
        ...
        loaders = []
        if len(self.train_dataset) > 0:
            loaders.append(self.train_dataloader())
        if len(self.val_dataset) > 0:
            loaders.append(self.val_dataloader())
        
        if len(loaders) == 0:
            # 如果train和val都是空的，使用test数据（推理模式）
    ...
    def node_types(self):
        ...
        if loader is None:
            # 如果所有数据集都是空的，返回默认值
            return torch.ones(8) / 8  # 假设8种原子类型
...
class Spec2MolDatasetInfos(AbstractDatasetInfos):
    ...
    def compute_input_output_dims(self, datamodule, extra_features, domain_features):
        ...
        elif len(datamodule.val_dataset) > 0:
            loader = datamodule.val_dataloader()
        elif len(datamodule.test_dataset) > 0:
            loader = datamodule.test_dataloader()
        else:
            raise ValueError("所有数据集都是空的，无法计算维度")
        
        example_batch_dict = next(iter(loader))
        example_batch = example_batch_dict['graph']
        
        # 从数据计算维度（训练/验证/测试模式）
        ex_dense, node_mask = utils.to_dense(example_batch.x, example_batch.edge_index, example_batch.edge_attr,
                                             example_batch.batch)
        example_data = {'X_t': ex_dense.X, 'E_t': ex_dense.E, 'y_t': example_batch_dict['y'], 'node_mask': node_mask}

        self.input_dims = {'X': example_batch['x'].size(1),
                           'E': example_batch['edge_attr'].size(1),
                           'y': example_batch_dict['y'].size(1) + 1}      # + 1 due to time conditioning
